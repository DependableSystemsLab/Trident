...
bool fi_trace_flag = false;
long long fi_trace_counter = 0;
...
bool preFunc(long llfi_index, unsigned opcode, unsigned my_reg_index, 
	...
	/* Inst counter*/
	/*
	if(curr_cycle > cycleInterval * cycleFactorCounter){
		cycleFactorCounter++;

		FILE *f = fopen("fi_cycle_count.txt", "w");
		fprintf(f, "%lld", curr_cycle);
		fclose(f);
	}
	*/
	...
	/*
	if(fi_trace_flag){
        	FILE *ft = fopen("test_fi_cycle_count.txt", "w");
	        fprintf(ft, "%lld", fi_trace_counter);
        	fclose(ft);
	}
	*/
	
/* JUSTIN: DETAILED COUNT */
/*
	 if(fi_trace_flag){
                fi_trace_counter++;
                FILE *f = fopen("fi_cycle_count.txt", "w");
                fprintf(f, "%lld", fi_trace_counter);
                fclose(f);
        }
*/
...
/*
	if(fi_trace_flag){
		...
		// Dump dyn trace after FI
		
		FILE *d = fopen("dyn_trace_after_fi.txt", "a");
                fprintf(d, "%ld\n", llfi_index);
                fclose(d);
				

		fi_trace_counter++;
		if(fi_trace_counter >= cycleInterval*10){
			cycleInterval *= 10;
			cycleFactor = 1;
		}

		if(fi_trace_counter > cycleInterval * cycleFactor){
	                cycleFactor++;
			FILE *f = fopen("fi_cycle_count.txt", "w");
        	        fprintf(f, "%lld", fi_trace_counter);
	               	fclose(f);
