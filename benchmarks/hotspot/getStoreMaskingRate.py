...
##########################
irPath = sys.argv[1]
fpMasking = 0.6602 # For hotspot: 52/64*(13/16)=0.6602
##########################
		...
		while len(depLoadList) != 0:
			...
			for diffLine in diffLines:
				if " " in diffLine and ("bio" in diffLine or "store" in diffLine or "call" in diffLine or "cmp" in diffLine):
					...
					if "call" in diffLine and ("printf" in diffLine or "fopen" in diffLine or "fputs" in diffLine or "fwrite" in diffLine or "_IO_putc" in diffLine):
...
with open("store_masking.txt", 'w') as sf:
	for storeIndex in storesList:
		cmpMasking = fpMasking + getMaskingRateFromInst(storeIndex)
		if cmpMasking > 1:
			cmpMasking = 1
		warMasking = 0
