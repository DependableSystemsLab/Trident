...
namespace{
	struct bishe_insert : public ModulePass{
		...
		virtual bool runOnModule(Module &M)
		{
			Constant *hookFunc;
			Constant *hookFunc2;
			hookFunc = M.getOrInsertFunction("profileInstruction", Type::getVoidTy(M.getContext()), Type::getInt64Ty(M.getContext()), NULL);
			...
			errs() << "Add profileInstruction function call to profile amount of instructions." << '\n';
			return false;
		}
		...
		virtual bool runOnBasicBlock(Function::iterator &BB, LLVMContext &context, Function* F)
		{
			for(BasicBlock::iterator BI = BB->begin(), BE = BB->end(); BI != BE; ++BI)           
			{
				...
				if(std::find(selectedInsts.begin(), selectedInsts.end(), opcode ) != selectedInsts.end()) {
					Value* indexValue = ConstantInt::get(Type::getInt64Ty(context), opcode);
