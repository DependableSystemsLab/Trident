...
namespace{
	struct bishe_insert : public ModulePass{
		...
		{
			Constant *hookFunc;
			hookFunc = M.getOrInsertFunction("duplicationCount", Type::getVoidTy(M.getContext()), Type::getInt64Ty(M.getContext()), NULL);
			...
			errs() << "Added Duplication and Count Function " << duplicatedInsts.size() << '\n';
			return false;
		}
		...
		virtual bool runOnBasicBlock(Function::iterator &BB, LLVMContext &context)
		{
			for(BasicBlock::iterator BI = BB->begin(), BE = BB->end(); BI != BE; ++BI)           
			{
				...
				if(std::find(selectedInsts.begin(), selectedInsts.end(), llfiIndex) != selectedInsts.end()) {
					...
					// Add duplicationCount function call
