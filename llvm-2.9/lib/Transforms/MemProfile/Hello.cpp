...
namespace{
	struct bishe_insert : public ModulePass{
		...
		{
			Constant *hookFunc;
			hookFunc = M.getOrInsertFunction("memProfile", Type::getVoidTy(M.getContext()), Type::getInt64Ty(M.getContext()), NULL);
			...
			errs() << "Add Counter for Memory Access (load and store)" << '\n';
			return false;
		}
