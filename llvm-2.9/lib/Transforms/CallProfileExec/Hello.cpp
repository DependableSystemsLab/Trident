...
namespace{
	struct bishe_insert : public ModulePass{
		...
		virtual bool runOnModule(Module &M)
		{
			...
			errs() << "Profiling Call Execution Probability ..." << '\n';
			return false;
		}
		virtual bool runOnBasicBlock(Function::iterator &BB, LLVMContext &context)
		{
			for(BasicBlock::iterator BI = BB->begin(), BE = BB->end(); BI != BE; ++BI)           
			{
				...
				if(opcode == 45){
					...
					insertCallProfileFunction(BI, indexValue);
				}
		...
		void insertCallProfileFunction(Instruction* callInst, Value* indexValue){
			// Creating function: void profileCall(long llfiIndex);
	
			BasicBlock::iterator nextInst = callInst;
			nextInst++;
			Instruction* instPos = callInst;
			...
			Constant* checker_handler_c = appModule->getOrInsertFunction("profileCall", checker_type);
