		...
		{			// for all blocks in the function
			...
			{		// for all instructions in a block
				for (BasicBlock::iterator IB = BBB->begin(), IE = BBB->end(); IB != IE; IB++)
				{
					...
					// doProfiling func hook
					/*
					Function *dphook;
					Constant *hookFunc;
					hookFunc = M.getOrInsertFunction("doProfiling", Type::getVoidTy(M.getContext()), Type::getInt32Ty(M.getContext()), NULL);
					dphook= cast<Function>(hookFunc);

					if(IB->getOpcode() != 28 && IB->getOpcode() != 2 && IB->getOpcode() != 1){
						Value* opcodeValue = ConstantInt::get(Type::getInt32Ty(M.getContext()), IB->getOpcode());
						CallInst* call_print = CallInst::Create(dphook, opcodeValue, "");
						BBB->getInstList().insert(IB, call_print);
					}
					*/
