...
namespace{
	struct bishe_insert : public ModulePass{
		...
		{
			Constant *hookFunc;
			hookFunc = M.getOrInsertFunction("checkpointInstTrack", Type::getVoidTy(M.getContext()), Type::getInt64Ty(M.getContext()), NULL);
			...
			for(Module::iterator F = M.begin(), E = M.end(); F!= E; ++F)
			{
				...
				if(functionName != "checkpointInstTrack"){
			...
			errs() << "Add Generic Checkpoint Tracker, fine grained tracking by 10^n" << '\n';
			return false;
		}
