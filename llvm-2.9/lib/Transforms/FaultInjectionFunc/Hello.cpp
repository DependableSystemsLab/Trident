...
namespace{
	struct bishe_insert : public ModulePass{
		...
		{
			Constant *hookFunc;
			hookFunc = M.getOrInsertFunction("faultInjectionFunc", Type::getVoidTy(M.getContext()), Type::getInt64Ty(M.getContext()), NULL);
			...
			for(Module::iterator F = M.begin(), E = M.end(); F!= E; ++F)
			{
				...
				if(functionName != "faultInjectionFunc" && functionName != "injectFault0" && functionName != "injectFault1"){
