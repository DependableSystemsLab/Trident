	...
	struct NLTSolver : public ModulePass{
		...
		virtual bool runOnBasicBlock(Function::iterator &BB, Function* f, long targetCmpIndex){
			for(BasicBlock::iterator BI = BB->begin(), BE = BB->end(); BI != BE; ++BI){
				if( getLLFIIndexofInst(BI) == targetCmpIndex){
					//errs() << "\nFound target CMP inst ...\n";
					processCmp(BI, f);
				}
			}
		}

		void processCmp(Instruction* targetCmpInst, Function* f){
			/*
			   LoopInfo &LI = getAnalysis<LoopInfo>(*f);
			   Loop* L = LI.getLoopFor(targetCmpInst->getParent());
			 */


			// Start from header
			BasicBlock *initBb = targetCmpInst->getParent();
			...
			while( !gQueue.empty() ){
				...
				if(std::find(headerBrIndexVec.begin(), headerBrIndexVec.end(), curBbBrIndex)!=headerBrIndexVec.end()){
					if(curBb != initNode->curBb){
						// Notify the target cmp index is 100% SDC-causing
						// DEBUG
						//errs() << "... Exit, loop contains ...\n";
						errs() << "Loop nested found ... \n";
						return;
					}
				}

				// Find prob. convergence point
